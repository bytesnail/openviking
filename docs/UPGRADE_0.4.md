...
我们踩过的 [issue #2718](https://github.com/volcengine/OpenViking/issues/2718)(v0.4.4 PR#2709 把 `Role(str,Enum)`→`Role(str)` 致 `ctx.role.value` 崩,经 MCP 入库全挂)是 **0.4.x auth 重构的回归**——当时降级 v0.4.3 规避;**修复 PR#2728 已在 v0.4.5(2026-06-24)正式发版,本仓 2026-06-25 已升级并端到端复测通过(见 `E2E_TESTING.md` §5.4)**。这正说明 0.4 的 auth/identity 大改是高风险区,升级必须端到端测。
...
5. **v0.4.5+ 已上**:PR#2728(role.value 修复)已在 v0.4.5 发版、本仓已升并端到端复测通过;后续若再升新版本,仍按 E2E_TESTING.md §8 checklist 走。
